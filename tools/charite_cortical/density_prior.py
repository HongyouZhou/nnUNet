"""Build, audit, and gate the Dataset778 cortical density prior."""
...
def _load_case(dataset: Path, identifier: str):
    ...
    if not (image.shape == semantic.shape == support.shape == validity.shape):
...
def calibrate(
    ...
    for fold, split in enumerate(splits):
        ...
        for identifier in train_cases:
            image, semantic, _, validity, _ = _load_case(dataset, identifier)
...
def _normal_surface(
    ...
    inner = support_mask & ~binary_erosion(
        support_mask,
...
def audit(dataset: Path, calibration_dir: Path, output: Path) -> dict[str, Any]:
    ...
    for fold, split in enumerate(splits):
        ...
        for identifier_value in split["val"]:
            ...
            image, semantic, support, validity, spacing = _load_case(dataset, identifier)
            valid = (validity & SEMANTIC_VALID_BIT) != 0
            separator = valid & (semantic == 2)
            surface = _normal_surface(support, separator, valid, spacing)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "calibrate":
        ...
        print(json.dumps({"density_prior_training_allowed": result["density_prior_training_allowed"]}))
        if not result["density_prior_training_allowed"]:
            return 2
