#!/usr/bin/env python3
"""
This script evaluates Visage outputs against ground truth instance segmentation.

Visage outputs separate binary masks for each bone instance (3_00_femur_1_seg.nii.gz, etc.)
This script:
1. Loads all instance masks for a case
...
Visage: /ssdArray/hongyou/dev/data/Visage Segmentierungen/{case_id}/*_seg.nii.gz
"""
...
def load_visage_masks(case_dir: str) -> tuple[np.ndarray | None, list[str]]:
    """
    Load all Visage binary masks for a case and convert to instance segmentation.
    Each *_seg.nii.gz file represents a single instance (no connected component analysis needed).
    
    Args:
        case_dir: Directory containing Visage outputs (e.g., .../1434/)
    ...
    # Find all *_seg.nii.gz files
    mask_files = sorted([f for f in case_path.glob("*_seg.nii.gz")])
    
    if not mask_files:
        logging.error(f"No mask files found in {case_dir}")
        return None, []
    
    logging.info(f"Found {len(mask_files)} instance mask files")
    ...
    # Process each instance mask file
    ...
    logging.info(f"Processed {len(loaded_files)} instance masks")
...
def evaluate_case(visage_dir: str,
                 gt_file: str,
                 case_id: str,
                 core_label: int = 2,
                 boundary_label: int = 1,
    ...
    """
    Evaluate Visage output for a single case.
    
    Args:
        visage_dir: Directory containing Visage outputs
    ...
    # Load Visage masks and convert to instance segmentation
    logging.info(f"Loading Visage masks from: {visage_dir}")
    pred_array, loaded_files = load_visage_masks(visage_dir)
    if pred_array is None:
        logging.error("Failed to load Visage masks")
        return None
...
def find_case_files(gt_dir: str, visage_dir: str) -> list[tuple[str, str, str]]:
    """
    Find matching case files in GT and Visage directories.
    
    Args:
        gt_dir: Ground truth directory
        visage_dir: Visage output directory
        
    Returns:
        List of (case_id, gt_file_path, visage_case_dir) tuples
    """
    gt_path = Path(gt_dir)
    visage_path = Path(visage_dir)
    
    if not gt_path.exists():
        logging.error(f"Error: GT directory does not exist: {gt_dir}")
        return []
    
    if not visage_path.exists():
        logging.error(f"Error: Visage directory does not exist: {visage_dir}")
        return []
    ...
    for case_dir in case_dirs:
        ...
        # Look for Visage output directory (directly under case_id)
        visage_case_dir = visage_path / case_id
        
        if not visage_case_dir.exists():
            logging.warning(f"Warning: No Visage output found for case {case_id}")
            continue
        
        # Check if there are any mask files
        mask_files = list(visage_case_dir.glob("*_seg.nii.gz"))
        
        if not mask_files:
            logging.warning(f"Warning: No mask files found in {visage_case_dir}")
            continue
        
        matched_files.append((case_id, str(gt_file), str(visage_case_dir)))
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate Visage outputs for PENGWIN bone fragment segmentation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Evaluate with default settings
  python evaluate_visage.py
  
  # Specify custom directories
  python evaluate_visage.py --gt_dir ~/dev/data/10_10 --visage_dir /path/to/visage
  
  # Use different IoU threshold for matching
  python evaluate_visage.py --iou_threshold 0.2
  
  # Save results to custom CSV file
  python evaluate_visage.py --output_file visage_results.csv
        """
    )
    
    # Directory arguments
    default_gt_dir = os.path.expanduser("~/dev/data/10_10")
    default_visage_dir = "/ssdArray/hongyou/dev/data/Visage Segmentierungen"
    default_output_file = "visage_evaluation_results.csv"
    
    parser.add_argument('--gt_dir', type=str, default=default_gt_dir,
                       help=f'Ground truth directory (default: {default_gt_dir})')
    parser.add_argument('--visage_dir', type=str, default=default_visage_dir,
                       help=f'Visage output directory (default: {default_visage_dir})')
    ...
    visage_dir = os.path.expanduser(args.visage_dir)
    
    logging.info("=" * 60)
    logging.info("Visage Segmentation Evaluation")
    logging.info("=" * 60)
    logging.info(f"Ground truth directory: {gt_dir}")
    logging.info(f"Visage directory: {visage_dir}")
    logging.info(f"IoU threshold: {args.iou_threshold}")
    
    # Find matching case files
    logging.info("\nDiscovering cases...")
    case_files = find_case_files(gt_dir, visage_dir)
    ...
    for case_id, gt_file, visage_case_dir in case_files:
        result = evaluate_case(
            visage_dir=visage_case_dir,
            gt_file=gt_file,
            case_id=case_id,
